...
// ******************************************************************
// *   NEPALI (BIKRAM SAMBAT / BS) DATE CONVERSION - DISPLAY ONLY   *
// ******************************************************************
//
// Purely additive: computes a BS-calendar equivalent for each AD date
// already being shown and inserts it as a separate element right next
// to the original. Never touches the original .dateformat_changer
// element's own text, so the existing date-format-switching feature
// above is completely unaffected. Ported from the same reference data
// used by the Python "nepali_datetime" library (calendar_bs.csv), so
// results match exactly what's used server-side elsewhere in the app.
const NEPALI_CALENDAR_START_YEAR = 1975;
...
const NEPALI_BS_REFERENCE_AD = new Date(Date.UTC(1918, 3, 13)); // 13 Apr 1918 = 1 Baisakh 1975 BS

function adToBs(isoDateStr) {
    // Expects a plain "YYYY-MM-DD" string (Django's default raw date
    // rendering). Returns null for anything else, including "None",
    // blank values, or dates outside the supported 1975-2100 BS range
    // -- callers should treat null as "don't show a BS date" rather
    // than an error, since a missing/odd value here should never break
    // the page.
    if (!isoDateStr || typeof isoDateStr !== "string") return null;
    const match = isoDateStr.trim().match(/^(\d{4})-(\d{2})-(\d{2})$/);
    if (!match) return null;

    const adDate = new Date(Date.UTC(
        parseInt(match[1], 10),
    ...
    const diffDays = Math.round((adDate - NEPALI_BS_REFERENCE_AD) / 86400000);
    if (diffDays < 0) return null;

    let year = NEPALI_CALENDAR_START_YEAR;
    let dayOfYear = diffDays + 1;

    while (true) {
        const months = NEPALI_CALENDAR_DATA[year - NEPALI_CALENDAR_START_YEAR];
        if (!months) return null; // past the end of the supported table
    ...
    return { year: year, month: month + 1, day: dayOfYear };
}

function formatBsDate(bs) {
    if (!bs) return "";
    return bs.day + " " + NEPALI_MONTH_NAMES[bs.month] + " " + bs.year;
}

function insertBsDateBadge(element, isoDateStr) {
    const bs = adToBs(isoDateStr);
    const $el = $(element);
    let $badge = $el.next(".nepali-bs-date");
    if (!bs) {
        // No valid/convertible date (e.g. "None", or outside supported
        // range) -- remove any stale badge rather than show something
        // wrong or leave an old value behind.
        $badge.remove();
        return;
    }
    const text = "(" + formatBsDate(bs) + ")";
    if ($badge.length) {
        $badge.text(text);
    } else {
        $("<span>", { class: "nepali-bs-date", text: text })
            .css({ "margin-left": "4px", "opacity": "0.75" })
            .insertAfter($el);
    }
}
...
if (window.CURRENT_LANGUAGE != 'de') {
    ...
    $(".dateformat_changer").each(function (index, element) {
        ...
        // Insert a BS (Nepali) date badge next to this element, computed
        // from the ORIGINAL raw text captured above, before it gets
        // reformatted below. See insertBsDateBadge() for details.
        insertBsDateBadge(element, currentDate);
