...
if env("DATABASE_URL", default=None):
    ...
    # "timeout" is a SQLite-only connection option. Passing it to other
    # backends (e.g. PostgreSQL/psycopg2) raises:
    #   "invalid dsn: invalid connection option "timeout""
    # which breaks every DB connection for anyone using the documented
    # Postgres configuration. Only apply it for SQLite; use the
    # correctly-named "connect_timeout" for everything else.
    _db_options = (
        {"timeout": 30}  # seconds to wait on a locked DB before raising OperationalError
        if _db_engine == "django.db.backends.sqlite3"
        else {"connect_timeout": 30}
    )
    DATABASES = {
        "default": {
            "ENGINE": _db_engine,
